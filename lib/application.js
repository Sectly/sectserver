...
function sectApp() {
  ...
  App.defaultSettings = () => {
    App.set('template', 'sectplating');
    App.set('sectplating', 'raw'); // raw or file
  ...
  App.handle = (request, response) => {
    ...
    res.header('X-Powered-By', 'SectServer');
    req.res = res;
    res.req = req;
    res.app = req.app = App;
    req.on('error', (err) => {
      logcon.error(`Problem With Request: ${err.message}`);
    });
    ...
    let queries = querystring.parse(req.query());
    var bodyplus = { queries: queries }
  ...
  App.err = function(req, res, next, err) {
    ...
    fs.readFile('./sectserver.log', (err, file) => {
      var date = new Date();
      let content = file + '\n' + new Date() + ' ---> ' + errstr;
      fs.writeFile('./sectserver.log', content,
        (err) => {
          if (err) throw err;
        });
    });
  ...
  App.listen = (port, callback) => {
    ...
    logcon.debug(`Server Listening On Port: ${port}`);
    callback && callback();
    return server;
  };
