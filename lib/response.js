...
const sectplating = require('./sectplating').sectplating;
...
Response.status = function status(sts) {
  this.setHeader('X-Powered-By', 'SectServer');
  if (typeof sts != 'number') {
    logcon.error('res.status() param must be a number, not a ' + typeof sts);
    sts = 302;
  }
  this.statusCode = sts;
  return this;
...
Response.sendStatus = function sendStatus(sts) {
  this.setHeader('X-Powered-By', 'SectServer');
  if (typeof sts !== 'number') {
    logcon.error('res.sendStatus() param must be a number, not a ' + typeof str);
    sts = 302;
  }
  switch (sts) {
    case 200:
...
Response.json = function json(obj) {
  ...
  if (typeof obj != 'object') {
    logcon.error('res.json() param must be a object, not a ' + typeof obj);
  }
...
Response.render = function(filename, options) {
  ...
  if (`${this.app.get('template')}` === `sectplating` || `${this.app.get('template')}` === `.sp` || `${this.app.get('template')}` === `sp`) {
    this.setHeader('X-Rendered-By', 'SectPlating');
    if (`${this.app.get('sectplating')}` == `file`) {
      let filecontent = fs.readFileSync(path);
      this.send(sectplating(filecontent, options));
    } else {
      this.send(sectplating(filename, options));
    }
  } else {
    cons[this.app.get('template')](path, options, (err, html) => {
      this.setHeader('X-Rendered-By', `${this.app.get('template')}`);
      if (err) {
        this.app.next(err);
      }
      else {
